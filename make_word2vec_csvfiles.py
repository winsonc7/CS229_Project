...
def main(data_file):
  ...
  for i in range(len(question)):
      question[i] = word_tokenize(clean_text(question[i]))

  model = Word2Vec(sentences=question, vector_size=VECSIZE, window=3, min_count=5, workers=4)
  ...
  columns = ['y'] + ['x' + str(i) for i in range(sentence_embeddings[0].shape[0] - 1)]
