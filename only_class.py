...
def filter_subjects(original_data, included_subjects):
    # Read the JSON file
    with open(original_data, 'r') as f:
        data = json.load(f)

    # Filter out samples with the excluded subjects
    filtered_data = [sample for sample in data if sample['subject'] in included_subjects]
...
# Usage: python only_class.py stem_data/stem.json chem math
if __name__ == "__main__":
    ...
    original = sys.argv[1]
    included_subjects = sys.argv[2:]

    # Filter out samples with the excluded subjects
    filter_subjects(original, included_subjects)